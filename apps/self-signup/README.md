...
This project is maintained by the Digital Acquisition squad. The goal is to add self-serve registration to eCompliance and other products. 
...
There are two QA environments available for self-signup:

1. `https://signup.qa.ecompliance.dev`

This environment gets deployed to automatically whenever the `develop` branch is updated. This environment would always have the latest copy of our app from the `develop` branch.

2. `https://signup.qa-da.ecompliance.dev` (QA-DA environment)

This environment is dedicated for the Digital Acquisition squad, and it is **never** automatically deployed to. 

To deploy here, you must run the `[qa] react-apps deploy DA` pipeline manually, after selecting the branch you want to deploy. 

This will give us the ability to deploy feature / release branches to this environment without affecting the main QA deployment.