...
function CreateAlcumusAdminUser() {
  ...
  cy.get('@orgnaizationDetails').then((val: any) => {
    ...
    cy.get('@Token').then((val1: any) => {
      cy.request({
        ...
        cy.writeFile('cypress/fixtures/users/Alcumus_Admin_User.json', ''); // clears the file before each tests
        cy.writeFile('cypress/fixtures/users/Alcumus_admin_User.json', {
          userEmail: userEmail,
          password: 'Test1234!',
          firstName: firstName,
          lastName: 'LastName',
...
Cypress.Commands.add('CreateAlcumusAdminUser', CreateAlcumusAdminUser);
